...
- Swiggy Builders Module Witness that assigns every Website Atlas module a live or fallback source state, proof links, CTA mapping, route optimization, risk boundary, next action, and operator or Swiggy gate.
- Swiggy Builders Navigation Witness that assigns every header, docs subnav, footer resource, and footer legal link a normalized URL, proof path, CTA live match, owner, and manual gate state.
...
- Swiggy Builders Coverage Receipt that reduces public pages, modules, CTAs, header, docs nav, footer, `llms` manifests, 69 docs pages, 35 reference tools, visual targets, fallback rows, missing rows, and external gates into one reviewer decision.
- Swiggy Builders AI Native Witness that ties the homepage AI-native promise to agent benchmarks, AI client configs, coding-agent governance, MCP resources/prompts, state orchestration, widget surfaces, innovation lanes, commercial-safety boundaries, and Swiggy gates.
- Swiggy Builders Enterprise Witness that ties the enterprise/platform-operator promise to delegated OAuth, tenant controls, quota and capacity review, support SLA, contracts, compliance, audit exports, brand governance, growth asks, and Swiggy approvals.
- Swiggy Builders Consumer Witness that ties the consumer AI-client path to AI clients, visual dish capture, voice commerce, nutrition/budget planning, household preferences, guest collaboration, luxury workspaces, confirmation safety, and Swiggy approvals.
- Swiggy Builders Tool Reference Witness that ties the official Food 14, Instamart 13, and Dineout 8 MCP tool references to contract rows, Tool Lab probes, recipe scenarios, docs twins, commercial-safety classes, and Swiggy live-credential gates.
...
- Swiggy Source Availability Audit that probes public Builders pages, docs roots, and `llms` manifests, then classifies live, fallback, watch, and blocked source states with proof links for temporary Swiggy glitch shells.
...
9. Show `/api/ready`, `/api/openapi.json`, `/api/swiggy-website-atlas`, `/api/swiggy-builders-module-intelligence`, `/api/swiggy-builders-module-witness`, `/api/swiggy-builders-navigation-witness`, `/api/swiggy-builders-launch-story`, `/api/swiggy-builder-intake`, `/api/swiggy-faq-policy`, `/api/swiggy-faq-resolution-center`, `/api/swiggy-builders-credential-sandbox-witness`, `/api/swiggy-builders-access-policy-witness`, `/api/swiggy-growth-partnership`, `/api/swiggy-talent-signal-center`, `/api/swiggy-conversion-center`, `/api/swiggy-showcase-submission-center`, `/api/swiggy-demo-evidence-director`, `/api/swiggy-partner-support-room`, `/api/channel-multimodal-studio`, `/api/swiggy-visual-dish-capture`, `/api/swiggy-voice-commerce-center`, `/api/nutrition-budget-intelligence`, `/api/household-preference-graph`, `/api/guest-collaboration-calendar`, `/api/luxury-experience-workspace`, `/api/reviewer-artifact-vault`, `/api/visual-qa-center`, `/api/submission-console`, `/api/access-submission-studio`, `/api/swiggy-docs-twin-explorer`, `/api/swiggy-upstream-watch`, `/api/swiggy-source-intelligence`, `/api/swiggy-deep-site-map`, `/api/swiggy-developer-quickstart`, `/api/swiggy-cta-execution-center`, `/api/swiggy-innovation-radar`, `/api/mcp/tool-lab`, `/api/mcp/tool-contract-matrix`, `/api/mcp/scenario-runner`, `/api/mcp/state-orchestrator`, `/api/mcp/commercial-action-guard`, `/api/mcp/backpressure-governor`, `/api/mcp/resource-prompt-studio`, `/api/mcp/widget-runtime`, `/api/swiggy-widget-experience-composer`, `/api/swiggy-hosted-widget-activation`, `/api/swiggy-agent-experience-benchmark`, `/api/swiggy-private-pilot-control-room`, `/api/swiggy-staging-replay`, `/api/mcp/staging-cutover`, `/api/swiggy-journey-compiler`, `/api/swiggy-access-dossier`, `/api/swiggy-access-evidence-matrix`, `/api/premium-use-case-studio`, `/api/premium-concierge-itinerary`, `/api/staging-certification-matrix`, `/api/auth/swiggy/status`, `/api/credential-onboarding`, `/api/sandbox-credential-workbench`, `/api/enterprise-delegated-auth`, `/api/enterprise-platform-center`, `/api/audit-ledger`, and `npm run verify:production`.
...
- Swiggy Builders Module Witness: `/api/swiggy-builders-module-witness`
- Swiggy Builders Navigation Witness: `/api/swiggy-builders-navigation-witness`
...
- Swiggy Builders Coverage Receipt: `/api/swiggy-builders-coverage-receipt`
...
- Swiggy Source Availability Audit: `/api/swiggy-source-availability-audit`
