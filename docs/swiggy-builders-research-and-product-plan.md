...
| What is Builders Club | Real products, AI-native agents, experimentation, support, and visibility. | AI Native Witness, Agent Experience Benchmark, AI Client Connect Kit, Coding Agent Governance, privacy controls, confirmation gates, production evidence, and Premium Use Case Studio |
...
| What you get | Live APIs, rate limits, support, co-branding, direct support, hiring visibility, and growth partnership. | Benefits Witness, Benefits Activation Center, Rate-limit plan, Support Bridge, Growth Partnership Center, Talent Signal Center, support report, product positioning, reviewer proof |
...
| Start | Developer, enterprise, consumer, coding-agent, and AI-client paths. | Developer path implemented; Consumer Witness and AI Client Connect Kit cover consumer clients, coding-agent rules, SDK auth modes, multimodal planning, premium workspaces, confirmation safety, and enterprise delegated-auth gates |
...
| Widgets | Use `/api/mcp/widget-runtime` semantic fallbacks and `/api/swiggy-widget-experience-composer` placement proof now; enable widget iframe headers after Swiggy hosting goes live |
...
| Swiggy Widget Experience Composer | `server/services/widgetExperienceComposer.ts` |
| Swiggy Hosted Widget Activation Center | `server/services/hostedWidgetActivation.ts` |
| Swiggy Agent Experience Benchmark | `server/services/agentExperienceBenchmark.ts` |
| Swiggy Private Pilot Control Room | `server/services/privatePilotControlRoom.ts` |
| Swiggy Staging Replay Center | `server/services/stagingReplayCenter.ts` |
...
| Journey Gate Center | `server/services/journeyGateCenter.ts` |
| Homepage Experience Center | `server/services/homepageExperienceCenter.ts` |
...
| Launch Center | Tool coverage, Tool Contract Matrix, Scenario Runner, State Orchestrator, Widget Runtime Center, Swiggy Widget Experience Composer, Swiggy Hosted Widget Activation Center, Swiggy Agent Experience Benchmark, Swiggy Private Pilot Control Room, Swiggy Staging Replay Center, Commercial Action Guard, Staging Cutover Rehearsal, Swiggy Handshake Doctor, Capability Registry, Resource & Prompt Studio, Website Atlas, Module Intelligence, Module Witness, Navigation Witness, Benefits Witness, AI Native Witness, Enterprise Witness, Consumer Witness, Tool Reference Witness, Journey Gates, Homepage Experience, Builder Intake, FAQ & Policy Center, Growth Partnership Center, Conversion Center, Showcase Submission Center, Submission Timeline Center, Partner Success Desk, Interaction QA Center, Channel & Multimodal Studio, Nutrition & Budget Intelligence, Household Preference Graph, Guest Collaboration & Calendar Center, Luxury Experience Workspace, Reviewer Artifact Vault, Review Decision Center, Visual QA Center, Access Evidence Matrix, Docs Coverage, Docs Twin Explorer, llms Manifest Verifier, Upstream Watch, Source Intelligence, Deep Site Map, Developer Quickstart, CTA Execution, Innovation Radar, Tool Lab, gateway, OAuth Status, Credential Cockpit, Credential Vault Center, Delegated Auth Center, Builders map, Support Bridge, go-live, observability, support |
...
- Keep `/api/swiggy-builders-benefits-witness` as source-of-truth evidence for What You Get benefit promises, source URLs, proof links, owner gates, route optimizations, risk boundaries, grouped readiness, and visual QA coverage.
- Keep `/api/swiggy-builders-ai-native-witness` as source-of-truth evidence for AI-native agent journeys, client targets, coding-agent rules, MCP resources/prompts, multi-turn state, widget surfaces, innovation lanes, commercial safety, source URLs, proof links, and Swiggy gates.
- Keep `/api/swiggy-builders-consumer-witness` as source-of-truth evidence for consumer AI-client setup, visual dish capture, voice commerce, nutrition/budget planning, household preferences, guest collaboration, luxury workspaces, confirmation safety, source URLs, proof links, and Swiggy approvals.
- Keep `/api/swiggy-builders-tool-reference-witness` as source-of-truth evidence for the official MCP tool reference layer: Food 14, Instamart 13, Dineout 8, contract rows, parameters, Tool Lab probes, recipe scenarios, docs coverage, docs twins, commercial-safety classes, source URLs, proof links, and Swiggy live-credential gates.
- Keep `/api/swiggy-builders-credential-sandbox-witness` as source-of-truth evidence for OAuth PKCE, DCR, credential vault redaction, handoff packets, sandbox readiness, staging credential drills, seeded smoke, 35-tool certification, cutover, live-signal calibration, source URLs, proof links, and Swiggy staging credential gates.
- Keep `/api/swiggy-builders-access-policy-witness` as source-of-truth evidence for production-access fields, review checks, policy rules, legal gates, official CTAs, attachments, browser runbook, brand/data controls, approval ownership, source URLs, proof links, and Swiggy production access approvals.
...
- Keep `/api/swiggy-builders-enterprise-witness` as source-of-truth evidence for enterprise delegated auth, tenant controls, quota and capacity review, support SLA, contract/compliance gates, audit exports, brand governance, growth asks, and Swiggy-owned approvals.
...
- Keep `/api/swiggy-widget-experience-composer` as source-of-truth evidence for premium widget placements, responsive gallery states, event-handler safety gates, and hosted-widget activation runbooks.
- Keep `/api/swiggy-hosted-widget-activation` as source-of-truth evidence for parent-origin policy, iframe sandbox, postMessage handshakes, fallback parity, telemetry redaction, visual proof, and hosted widget URL gates.
- Keep `/api/swiggy-agent-experience-benchmark` as source-of-truth evidence for best-in-class journey scores, UX acceptance criteria, competitive moats, route savings, multimodal continuity, and staged innovation gates.
- Keep `/api/swiggy-private-pilot-control-room` as source-of-truth evidence for private-pilot cohorts, consent artifacts, success metrics, telemetry fields, support paths, operator runbooks, and Swiggy staging replay gates.
- Keep `/api/swiggy-staging-replay` and `/api/swiggy-staging-replay/run` as source-of-truth evidence for allowlisted safe replay probes, dry-run versus credentialed execution, commercial-action blocks, response hashes, token redaction, and Swiggy staging handoff copy.
