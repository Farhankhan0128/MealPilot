...
The verifier also validates `/api/swiggy-builders-navigation-witness` for global header, docs subnav, footer program, footer resources, footer legal links, normalized URLs, CTA live matches, proof links, owners, and manual legal/email gates.
...
The verifier also validates `/api/swiggy-builders-tool-reference-witness` for the official Swiggy MCP reference layer, including Food 14, Instamart 13, Dineout 8, contract rows, parameters, Tool Lab probes, recipe scenarios, docs coverage, docs twins, commercial-safety classes, proof links, and Swiggy-owned live credential gates.
...
`/api/swiggy-credential-issuance/state` stores only redacted issuance metadata after Swiggy responds: DCR approval time, client-id configured flag, staging credential issue time, Food/Instamart/Dineout seeded-user receipt booleans, support thread id, token-expiry recorded flag, first-read readiness, notes, and update timestamp. `/api/swiggy-credential-readiness-dossier` and `/api/swiggy-credential-readiness-dossier/rehearse` turn that state into a source-freeze-aware credential receipt room with public homepage 3-server and 18+ API-tool signals, 35-tool manifest certification, access submission receipts, DCR/vault/staging/prod gates, proof commands, and no token/client-secret/auth-code/PKCE leakage.
...
`/api/swiggy-source-freeze-diff` is the final source-freeze proof before demo recording or access submission. It composes live Builders Page Mesh, Website Atlas header/footer/CTA counts, llms/docs coverage, 35-tool reference parity, Access Evidence Matrix, Access Policy Witness, Credential Sandbox Witness, Builder Packet Export, Upstream Watch, proof commands, missing inputs, and the operator browser re-browse gate. `POST /api/swiggy-source-freeze-diff/freeze` reruns the same check for pre-demo, pre-access-submission, or post-source-change modes without submitting external state; when the operator adds a metadata-only browser re-browse receipt, the browser row becomes matched and the freeze can return `ready_to_freeze`.
...
`/api/swiggy-builders-navigation-witness` joins Website Atlas and CTA Live Audit into one header/docs/footer evidence receipt with normalized URLs, link groups, proof links, owners, manual legal/email gates, and reviewer next actions.
...
`/api/swiggy-builders-completion-ledger` composes the major proof centers into a full-objective audit board. It maps public site coverage, developer and enterprise tracks, all MCP servers and tools, route optimization, premium frontend, backend APIs, logging, tracing, tests, docs, access submission, sandbox credentials, safety, reviewer packet, launch readiness, frontend surfaces, backend endpoints, docs proof, test proof, owners, proof links, and remaining operator or Swiggy gates.

`/api/swiggy-builders-coverage-receipt` composes Website Atlas, Page Mesh, CTA Live Audit, Docs Coverage, Source Availability, Tool Parity, Visual QA, proof links, fallback rows, missing rows, and external Swiggy gates into one compact `complete_coverage`, `coverage_watch`, or `blocked_drift` receipt for the final reviewer packet.

`/api/swiggy-source-availability-audit` probes public Builders pages, docs roots, and `llms` manifests, classifies live content versus temporary-glitch fallback, blocked fetches, and unexpected bodies, then links every row to Website Atlas, Page Mesh, Docs Coverage, Docs Twin Explorer, and packet proof routes for reviewer-safe disclosure.
...
- Swiggy Builders Navigation Witness gives every header, docs, footer resource, and footer legal link a row-level proof path, normalized URL, CTA live match, owner, and manual gate state.
...
- Swiggy Source Availability Audit probes every public source row, docs root, and llms manifest for live, fallback, watch, or blocked state while preserving proof links for temporary Swiggy glitch shells.
- Swiggy Builders Coverage Receipt reduces public pages, modules, CTAs, header, docs nav, footer, llms manifests, 69 docs pages, 35 tools, visual targets, fallback rows, missing rows, and external gates into one verifier-backed decision.
- Swiggy Source Freeze Diff composes live page mesh, source intelligence, llms/docs coverage, access evidence, Access Policy Witness, Credential Sandbox Witness, builder packet export, upstream watch, proof commands, and browser re-browse gates into one final pre-submission freeze decision.
- Source Freeze Diff accepts a metadata-only browser re-browse receipt on explicit POST rehearsals so the final visual browser check is auditable without storing screenshots, cookies, tokens, profile data, or page HTML.
...
- Swiggy Showcase Submission Center packages pitch blocks, demo storyboard, metric pack, visual-gallery links, outreach copy, operator inputs, and Swiggy approval gates for a feature-ready review packet; the reviewer vault, builder packet export, and production launch bundle now carry it as final Show Us What You Built handoff evidence.
- Reviewer Artifact Vault promotes a dedicated partner/signal operations section for the Operating Contract, Brand Compliance Kit, Partner Success Desk, and Live Signal Calibration Center, while the launch bundle and reviewer handoff email also link Submission Timeline, growth, benefits, support, traffic, quota, and governance evidence for Swiggy support/channel review.
- Reviewer Artifact Vault promotes a dedicated MCP client-readiness section for AI Client Connect, Coding Agent Governance, MCP Gateway, Capability Registry, Handshake Doctor, llms Manifest Verifier, Tool Parity Auditor, and Docs Coverage, while the builder packet and launch handoff email keep those proof links together for Swiggy reviewer setup.
- Reviewer Artifact Vault promotes a dedicated source-freeze/live-site QA section for Builders Site Parity, Page Mesh, CTA Live Audit, Interaction QA, Source Freeze Diff, Credential Vault, and Staging Seed & Smoke, while the builder packet and launch handoff email keep those proof links together before demo recording or access submission.
- Reviewer Artifact Vault promotes a dedicated executable CTA rehearsal matrix for access submission, reviewer packet, visual QA, docs/source, AI client, brand, interaction, growth, support, showcase, channel, nutrition, household, guest, and luxury composer endpoints, while the builder packet and launch handoff email prove local buttons produce safe dry-run evidence.
- Reviewer Artifact Vault promotes a dedicated runtime-governance section for health, readiness, config, privacy, storage, reviewer proof, evaluation, demo, submission, support-report, session replay, planner, confirmation, and go-live endpoints, while the builder packet and launch handoff email prove the product can be operated, exported, restored, audited, and replayed safely.
...
- Credential Readiness Dossier reports durable redacted issuance receipt state, source-freeze posture, homepage-vs-manifest source signals, proof commands, rehearsal decisions, and Swiggy/operator gates; the reviewer vault, builder packet export, and production launch bundle now carry that dossier and the redacted issuance-state link as final handoff evidence.
