...
flowchart LR
  ...
  API --> Customization["Customization Studio"]
...
- Launch Center with MCP coverage, Journey Compiler, Access Dossier, Access Evidence Matrix, Growth Partnership Center, Conversion Center, Journey Gates, Homepage Experience, Channel & Multimodal Studio, Visual Dish Capture Center, Voice Commerce Rehearsal Center, Quality Loop Center, Ritual Autopilot Center, Payment Truth Center, Meal Window Intelligence, Customization Studio, Nutrition & Budget Intelligence, Household Preference Graph, Guest Collaboration & Calendar Center, Luxury Experience Workspace, Reviewer Artifact Vault, Credential Readiness Dossier, Review Decision Center, Visual QA Center, Premium Use Case Studio, Staging Cutover Rehearsal, Swiggy Staging Replay Center, Swiggy Staging Credential Drill Center, Staging Certification Matrix, Brand Compliance Kit, Capability Registry, Resource & Prompt Studio, Tool Contract Matrix, Widget Runtime Center, Swiggy Widget Experience Composer, Swiggy Hosted Widget Activation Center, Swiggy Agent Experience Benchmark, Swiggy Private Pilot Control Room, Commercial Action Guard, Website Atlas with access-page and launch-blog coverage, Module Intelligence, Module Witness, Navigation Witness, Benefits Witness, AI Native Witness, Enterprise Witness, Consumer Witness, Tool Reference Witness, Builders Launch Story Center, Docs Coverage, Docs Twin Explorer, Upstream Watch, Source Intelligence, Deep Site Map, Developer Quickstart, CTA Execution, Innovation Radar, Tool Lab, Credential Cockpit, Support Bridge, chat/voice response contracts, go-live checks, observability metrics, rollout plan, and support report generation.
...
- Exposes `/api/swiggy-widget-experience-composer` for premium Swiggy widget placements, responsive gallery states, semantic fallback renderers, event-handler safety gates, and hosted-widget activation runbooks.
- Exposes `/api/swiggy-hosted-widget-activation` for hosted iframe parent-origin policy, sandbox contracts, postMessage handshakes, fallback parity, redaction telemetry, visual proof, and Swiggy-owned hosted URL gates.
- Exposes `/api/swiggy-agent-experience-benchmark` for best-in-class Swiggy journey scoring across route speed, trust, personalization, multimodal continuity, resilience, commercial safety, UX acceptance gates, and innovation moats.
- Exposes `/api/swiggy-private-pilot-control-room` for real-user private pilot cohorts, consent artifacts, success metrics, telemetry targets, support paths, operator runbooks, and Swiggy staging replay gates.
- Exposes `/api/swiggy-staging-replay` and `/api/swiggy-staging-replay/run` for safe staging replay probes, mock dry-run evidence, credentialed staging execution, response hashes, token redaction telemetry, and commercial-action blocking.
...
- Exposes `/api/swiggy-faq-resolution-center` as the reviewer answer layer for FAQ questions, policy rules, proof routes, activation CTAs, script steps, support contact, and operator or Swiggy gates.
...
- Exposes `/api/swiggy-talent-signal-center` as the builder portfolio layer for standout-project, demo, GitHub, architecture, metric, visual, outreach, and hiring-readiness proof with Swiggy-owned recruiting gates.
- Exposes `/api/swiggy-conversion-center` as the final Builders CTA layer for What Will You Cook, Start Building, See What's Possible, Request Access, Send Us a Demo, builders@swiggy.in, `llms.txt`, `llms-full.txt`, proof bundles, operator runbook, and go-live gates.
- Exposes `/api/swiggy-builders-module-intelligence` as the page-module readiness layer for owner, audience, product promise, Swiggy surface, MealPilot proof, route optimization, risk boundary, CTA mapping, module journeys, and external gates.
- Exposes `/api/swiggy-builders-module-witness` as the row-level module evidence layer for Page Mesh source state, Website Atlas fallback disclosure, owner gates, CTA mapping, proof links, route optimization, risk boundary, and next action.
- Exposes `/api/swiggy-builders-navigation-witness` as the row-level navigation evidence layer for global header, docs subnav, footer resources, footer legal links, CTA live matches, owners, proof links, and manual gates.
- Exposes `/api/swiggy-builders-journey-gates` as the official Builders five-step gate layer for Start Building, Apply for Prod Access, Quick Review, Go Live, and Show Us What You Built with criteria, proofs, telemetry, blockers, and external approvals.
- Exposes `/api/swiggy-builders-homepage-experience` as the homepage section layer for header, hero, how-it-works, benefits, guidelines, FAQ, final CTA, and footer proof with mobile checks and reviewer checks.
...
- Exposes `/api/swiggy-customization-studio` and `/api/swiggy-customization-studio/validate` as the exact-choice layer for Food add-ons, variants, Instamart pack sizes, allergy cautions, voice-safe choice limits, and cart readbacks before mutation.
...
- Exposes `/api/reviewer-artifact-vault` as the access-submission manifest for proof links, OpenAPI, smoke commands, screenshot targets, Credential Readiness Dossier, Showcase Submission Center, redacted issuance state, demo-video checklist, logs, traces, redaction rules, support context, and Swiggy handoff copy.
...
- Exposes `/api/swiggy-builders-review-decision` as the Swiggy approval-readiness decision board for fit, demo, security, API coverage, live-source review, credentials, ops, go-live gates, reviewer Q&A, proof links, and operator-vs-Swiggy blockers.
...
- Generates Swiggy widget contracts with iframe sizing, postMessage events, sandbox policy, origin verification, semantic fallbacks, and premium experience placements for access-review screenshots.
...
- `server/services/customizationStudio.ts`
...
`/api/swiggy-credential-handoff-center` composes the onboarding, vault, sandbox, staging drill, cutover, and certification reports into one credential handoff room. It keeps each phase owner-assigned, preserves Swiggy staging and production gates, and points reviewers to the exact API evidence for local proof, OAuth PKCE, seeded smoke, and promotion.

`/api/swiggy-benefits-activation-center` composes Growth Partnership, Quota Negotiation, Brand Compliance, Partner Success, Partner Support, and Enterprise Platform evidence into one Builders-benefit activation layer. It maps live APIs, quota expansion, technical support, co-branding, showcase visibility, hiring visibility, growth partnership, and enterprise support to proof routes, CTAs, owners, and external gates.

`/api/swiggy-builders-benefits-witness` exposes the read-only witness for that layer, tying each What You Get promise to source URLs, proof links, owner gates, route optimizations, risk boundaries, and Launch Center evidence.

`/api/swiggy-builders-ai-native-witness` exposes the read-only witness for AI-native agent depth, tying benchmark journeys, AI client targets, coding-agent governance, resources/prompts, multi-turn state, widgets, innovation, and commercial safety to source URLs, proof links, route optimizations, risk boundaries, and Swiggy gates.

`/api/swiggy-builders-enterprise-witness` exposes the read-only witness for enterprise platform depth, tying delegated OAuth, tenant controls, quota and capacity review, support SLA, contracts/compliance, audit exports, brand governance, and growth asks to source URLs, proof links, route optimizations, risk boundaries, and Swiggy approvals.

`/api/swiggy-builders-consumer-witness` exposes the read-only witness for consumer AI-client depth, tying AI client setup, visual dish capture, voice commerce, nutrition and budget planning, household preferences, guest collaboration, luxury workspaces, and confirmation safety to source URLs, proof links, route optimizations, risk boundaries, and Swiggy approvals.

`/api/swiggy-builders-tool-reference-witness` exposes the read-only witness for the official MCP reference layer, tying Food 14, Instamart 13, Dineout 8, contract rows, parameters, Tool Lab probes, official recipe scenarios, docs coverage, docs twins, and commercial safety classes to source URLs, proof links, route optimizations, risk boundaries, and Swiggy live-credential gates.
