  ...
  return (
    <section className="analysis-panel launch-panel" id="launch-center">
      ...
      <div className="launch-grid">
        ...
        <article className="module-witness-card">
          <div className="mini-heading">
            <ClipboardCheck aria-hidden="true" />
            <strong>Module Witness</strong>
          </div>
          <span>
            {moduleWitness
              ? `${moduleWitness.score}/100, ${moduleWitness.totals.proven + moduleWitness.totals.fallback}/${moduleWitness.totals.modules} modules witnessed`
              : "Witnessing live, fallback, CTA, proof, and gate coverage across Builders modules"}
          </span>
          <div className="module-witness-grid">
            <div>
              <strong>{moduleWitness?.totals.pages ?? 0}</strong>
              <span>Pages</span>
            </div>
            <div>
              <strong>{moduleWitness?.totals.fallback ?? 0}</strong>
              <span>Fallback</span>
            </div>
            <div>
              <strong>{(moduleWitness?.totals.operatorGates ?? 0) + (moduleWitness?.totals.swiggyGates ?? 0)}</strong>
              <span>Gates</span>
            </div>
            <div>
              <strong>{moduleWitness?.totals.proofLinks ?? 0}</strong>
              <span>Proofs</span>
            </div>
          </div>
          <ul className="compact-status-list">
            {(moduleWitness?.pageSummaries ?? []).slice(0, 5).map((summary) => (
              <li
                key={summary.pageId}
                data-status={
                  summary.swiggyGates > 0 || summary.watch > 0
                    ? "blocked"
                    : summary.fallback > 0 || summary.operatorGates > 0
                      ? "watch"
                      : "healthy"
                }
              >
                <span>{summary.title}</span>
                <strong>{summary.proven + summary.fallback}/{summary.modules}</strong>
              </li>
            ))}
          </ul>
          <div className="source-intelligence-actions" aria-label="Module witness links">
            <a href="/api/swiggy-builders-module-witness" target="_blank" rel="noreferrer">
              Witness API
            </a>
            <a href="/api/swiggy-builders-module-intelligence" target="_blank" rel="noreferrer">
              Modules
        ...
        <article className="benefits-witness-card">
          <div className="mini-heading">
            <Sparkles aria-hidden="true" />
            <strong>Benefits Witness</strong>
          </div>
          <span>
            {benefitsWitness
              ? `${benefitsWitness.score}/100, ${benefitsWitness.totals.proven}/${benefitsWitness.totals.rows} benefits proven`
              : "Witnessing live APIs, quota, support, co-branding, growth, showcase, hiring, and enterprise benefits"}
          </span>
          <div className="benefits-witness-grid">
            <div>
              <strong>{benefitsWitness?.totals.rows ?? 0}</strong>
              <span>Benefits</span>
            </div>
            <div>
              <strong>{benefitsWitness?.totals.swiggyMcpServers ?? 0}</strong>
              <span>Servers</span>
            </div>
            <div>
              <strong>{(benefitsWitness?.totals.operatorGates ?? 0) + (benefitsWitness?.totals.swiggyGates ?? 0)}</strong>
              <span>Gates</span>
            </div>
            <div>
              <strong>{benefitsWitness?.totals.proofLinks ?? 0}</strong>
              <span>Proofs</span>
            </div>
          </div>
          <ul className="compact-status-list">
            {(benefitsWitness?.groups ?? []).map((group) => (
              <li
                key={group.id}
                data-status={group.swiggyGates > 0 ? "blocked" : group.operatorGates > 0 || group.watch > 0 ? "watch" : "healthy"}
              >
                <span>{group.label}</span>
                <strong>{group.proven}/{group.rows}</strong>
              </li>
            ))}
          </ul>
          <div className="source-intelligence-actions" aria-label="Benefits witness links">
            <a href="/api/swiggy-builders-benefits-witness" target="_blank" rel="noreferrer">
              Benefits API
            </a>
        ...
        <article className="consumer-witness-card">
          <div className="mini-heading">
            <Sparkles aria-hidden="true" />
            <strong>Consumer Witness</strong>
          </div>
          <span>
            {consumerWitness
              ? `${consumerWitness.score}/100, ${consumerWitness.totals.proven + consumerWitness.totals.ready}/${consumerWitness.totals.rows} consumer lanes ready`
              : "Witnessing AI clients, visual input, voice, nutrition, households, guests, premium workspaces, and confirmation safety"}
          </span>
          <div className="consumer-witness-grid">
            <div>
              <strong>{consumerWitness?.totals.clientTargets ?? 0}</strong>
              <span>Clients</span>
            </div>
            <div>
              <strong>{consumerWitness?.totals.voiceRoutes ?? 0}</strong>
              <span>Voice</span>
            </div>
            <div>
              <strong>{consumerWitness?.totals.householdSignals ?? 0}</strong>
              <span>Signals</span>
            </div>
            <div>
              <strong>{consumerWitness?.totals.proofLinks ?? 0}</strong>
              <span>Proofs</span>
            </div>
          </div>
          <ul className="compact-status-list">
            {(consumerWitness?.groups ?? []).map((group) => (
              <li key={group.id} data-status={group.gates > 0 || group.watch > 0 ? "watch" : "healthy"}>
                <span>{group.label}</span>
                <strong>{group.proven + group.ready}/{group.rows}</strong>
              </li>
          ...
          <div className="source-intelligence-actions" aria-label="Consumer witness links">
            <a href="/api/swiggy-builders-consumer-witness" target="_blank" rel="noreferrer">
              Consumer API
            </a>
            <a href="/api/ai-client-connect-kit" target="_blank" rel="noreferrer">
              Clients
        ...
        <article className="credential-sandbox-witness-card">
          <div className="mini-heading">
            <LockKeyhole aria-hidden="true" />
            <strong>Credential Sandbox Witness</strong>
          </div>
          <span>
            {credentialSandboxWitness
              ? `${credentialSandboxWitness.score}/100, ${credentialSandboxWitness.totals.proven + credentialSandboxWitness.totals.ready}/${credentialSandboxWitness.totals.rows} credential lanes ready`
              : "Witnessing OAuth, DCR, vault redaction, credential handoff, sandbox staging, seed smoke, and cutover gates"}
          </span>
          <div className="credential-sandbox-witness-grid">
            <div>
              <strong>{credentialSandboxWitness?.totals.authLifecycleLanes ?? 0}</strong>
              <span>Auth lanes</span>
            </div>
            <div>
              <strong>{credentialSandboxWitness?.totals.vaultSecrets ?? 0}</strong>
              <span>Secrets</span>
            </div>
            <div>
              <strong>{credentialSandboxWitness?.totals.stagingDrills ?? 0}</strong>
              <span>Drills</span>
            </div>
            <div>
              <strong>{credentialSandboxWitness?.totals.certificationTools ?? 0}</strong>
              <span>Tools</span>
            </div>
          </div>
          <ul className="compact-status-list">
            {(credentialSandboxWitness?.groups ?? []).map((group) => (
              <li key={group.id} data-status={group.gates > 0 || group.watch > 0 ? "watch" : "healthy"}>
                <span>{group.label}</span>
                <strong>{group.proven + group.ready}/{group.rows}</strong>
              </li>
          ...
          <div className="source-intelligence-actions" aria-label="Credential sandbox witness links">
            <a href="/api/swiggy-builders-credential-sandbox-witness" target="_blank" rel="noreferrer">
              Credential API
            </a>
            <a href="/api/sandbox-credential-workbench" target="_blank" rel="noreferrer">
              Sandbox
            </a>
