...
describe("MealPilot app", () => {
  ...
  it("renders the premium portal shell, mobile navigation, and CTA feedback", async () => {
    ...
    expect(screen.getByText("Coverage Receipt")).toBeInTheDocument();
    expect(screen.getAllByRole("link", { name: "Receipt API" })).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ href: expect.stringContaining("/api/swiggy-builders-coverage-receipt") }),
      ]),
    );
    expect(screen.getByText("Module Witness")).toBeInTheDocument();
    expect(screen.getAllByRole("link", { name: "Witness API" })).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ href: expect.stringContaining("/api/swiggy-builders-module-witness") }),
      ]),
    );
    expect(screen.getByText("Navigation Witness")).toBeInTheDocument();
    expect(screen.getAllByRole("link", { name: "Nav API" })).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ href: expect.stringContaining("/api/swiggy-builders-navigation-witness") }),
      ]),
    );
    expect(screen.getByText("Benefits Witness")).toBeInTheDocument();
    ...
    expect(screen.getByText("Enterprise Witness")).toBeInTheDocument();
    expect(screen.getAllByRole("link", { name: "Enterprise API" })).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ href: expect.stringContaining("/api/swiggy-builders-enterprise-witness") }),
      ]),
    );
    expect(screen.getByText("Consumer Witness")).toBeInTheDocument();
    expect(screen.getAllByRole("link", { name: "Consumer API" })).toEqual(
      expect.arrayContaining([
    ...
    expect(screen.getByText("Credential Sandbox Witness")).toBeInTheDocument();
    expect(screen.getAllByRole("link", { name: "Credential API" })).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ href: expect.stringContaining("/api/swiggy-builders-credential-sandbox-witness") }),
      ]),
    );
    expect(screen.getByText("Access Policy Witness")).toBeInTheDocument();
    expect(screen.getAllByRole("link", { name: "Access API" })).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ href: expect.stringContaining("/api/swiggy-builders-access-policy-witness") }),
      ]),
    );
    ...
    expect(screen.getAllByRole("link", { name: "Availability API" })).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ href: expect.stringContaining("/api/swiggy-source-availability-audit") }),
      ]),
    );
