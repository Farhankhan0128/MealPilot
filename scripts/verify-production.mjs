...
assert(
  openApi.paths["/api/swiggy-tool-parity-auditor"]?.get?.summary?.includes("tool parity") &&
    openApi.paths["/api/swiggy-tool-parity-auditor"]?.get?.responses?.["200"]?.description?.includes("Instamart 13"),
...
assert(
  openApi.paths["/api/swiggy-builders-navigation-witness"]?.get?.summary?.includes("Navigation Witness") &&
    openApi.paths["/api/swiggy-builders-navigation-witness"]?.get?.responses?.["200"]?.description?.includes("footer legal"),
  "OpenAPI Builders navigation witness contract is missing",
);
assert(
  openApi.paths["/api/swiggy-builders-benefits-witness"]?.get?.summary?.includes("Benefits Witness") &&
    openApi.paths["/api/swiggy-builders-benefits-witness"]?.get?.responses?.["200"]?.description?.includes("co-branding"),
...
const navigationWitness = await request("/api/swiggy-builders-navigation-witness");
assert(navigationWitness.navigationWitness.score >= 90, "Builders navigation witness score is below target");
assert(
  ["navigation_ready", "navigation_watch"].includes(navigationWitness.navigationWitness.decision) &&
    navigationWitness.navigationWitness.totals.rows >= 20 &&
    navigationWitness.navigationWitness.totals.headerLinks === 7 &&
    navigationWitness.navigationWitness.totals.docsLinks === 5 &&
    navigationWitness.navigationWitness.totals.footerLinks >= 8 &&
    navigationWitness.navigationWitness.totals.blocked === 0,
  "Builders navigation witness totals are incomplete",
);
assert(
  ["global_header_home", "docs_subnav_docs_reference", "footer_resources_llms", "footer_legal_privacy"].every((id) =>
    navigationWitness.navigationWitness.rows.some((row) => row.id === id),
  ),
  "Builders navigation witness key rows are missing",
);
assert(
  navigationWitness.navigationWitness.rows.some((row) => row.id === "footer_legal_privacy" && row.status === "manual_gate") &&
    navigationWitness.navigationWitness.groups.some((group) => group.id === "footer_legal" && group.manualGates >= 2) &&
    navigationWitness.navigationWitness.commands.some((command) =>
      command.command.includes("/api/swiggy-builders-navigation-witness"),
    ) &&
    navigationWitness.navigationWitness.assertions.some((assertion) => assertion.includes("global header")),
  "Builders navigation witness gates or commands are missing",
);

const benefitsWitness = await request("/api/swiggy-builders-benefits-witness");
assert(benefitsWitness.benefitsWitness.score >= 80, "Builders benefits witness score is below target");
assert(
  ["benefits_ready", "benefits_watch"].includes(benefitsWitness.benefitsWitness.decision) &&
    benefitsWitness.benefitsWitness.totals.rows >= 9 &&
    benefitsWitness.benefitsWitness.totals.proven >= 2 &&
    benefitsWitness.benefitsWitness.totals.swiggyGates >= 2 &&
    benefitsWitness.benefitsWitness.totals.proofLinks >= 20 &&
    benefitsWitness.benefitsWitness.totals.swiggyMcpServers === 3 &&
    benefitsWitness.benefitsWitness.totals.officialTools === 35 &&
    benefitsWitness.benefitsWitness.totals.activationCtas === 6,
  "Builders benefits witness totals are incomplete",
);
assert(
  [
    "live_api_access",
    ...
    benefitsWitness.benefitsWitness.rows.some(
      (row) => row.id === id && row.proofLinks.length > 0 && row.routeOptimization.length > 0 && row.riskBoundary.length > 0,
    ),
  ),
  "Builders benefits witness key rows are missing",
);
assert(
  ["platform", "operations", "growth"].every((id) => benefitsWitness.benefitsWitness.groups.some((group) => group.id === id)) &&
    benefitsWitness.benefitsWitness.commands.some((command) =>
      command.command.includes("/api/swiggy-builders-benefits-witness"),
    ) &&
    benefitsWitness.benefitsWitness.assertions.some((assertion) => assertion.includes("What You Get")) &&
    benefitsWitness.benefitsWitness.externalGates.some((gate) => gate.includes("production credentials")),
...
assert(journeyGates.journeyGates.score >= 80, "Builders journey gate score is below target");
...
assert(
  ...
  "Builders journey gate criteria are incomplete",
);
assert(journeyGates.journeyGates.totals.proofLinks >= 20, "Builders journey proof links are incomplete");
assert(journeyGates.journeyGates.totals.telemetryLinks >= 10, "Builders journey telemetry links are incomplete");
assert(
  ["start_building", "apply_prod_access", "quick_review", "go_live", "show_built"].every((id) =>
    journeyGates.journeyGates.gates.some((gate) => gate.id === id),
  ),
  "Builders journey gate ids are missing",
);
assert(
  journeyGates.journeyGates.gates.some(
    (gate) =>
      gate.id === "go_live" &&
  ...
  "Builders journey go-live gate proof is missing",
);
assert(
  journeyGates.journeyGates.operatorRunbook.map((step) => step.sequence).join(",") === "1,2,3,4,5" &&
...
assert(
  ...
  "reviewer artifact vault showcase submission artifact is missing",
);
assert(
  reviewerArtifactVault.reviewerArtifactVault.artifactSections.some((section) =>
    section.artifacts.some(
...
assert(
  ...
  "reviewer artifact vault access submission studio artifact is missing",
);
assert(
  reviewerArtifactVault.reviewerArtifactVault.artifactSections.some((section) =>
    section.artifacts.some(
...
assert(
  visualQa.visualQa.targetGroups.some((group) =>
    group.targets.some((target) => target.id === "navigation_witness_card" && target.selector === ".navigation-witness-card"),
  ),
  "visual QA Navigation Witness target is missing",
);
assert(
  visualQa.visualQa.targetGroups.some((group) =>
...
assert(
  ...
  "visual QA tool parity target is missing",
);
assert(
  visualQa.visualQa.targetGroups.some((group) =>
...
assert(toolParityAuditor.toolParityAuditor.score >= 95, "Swiggy tool parity auditor score is below target");
assert(toolParityAuditor.toolParityAuditor.totals.liveReferenceTools === 35, "Swiggy tool parity live references are incomplete");
assert(toolParityAuditor.toolParityAuditor.totals.localContracts === 35, "Swiggy tool parity local contracts are incomplete");
assert(toolParityAuditor.toolParityAuditor.totals.matchedTools === 35, "Swiggy tool parity matched tools are incomplete");
assert(toolParityAuditor.toolParityAuditor.totals.missingContracts === 0, "Swiggy tool parity has missing contracts");
assert(toolParityAuditor.toolParityAuditor.totals.extraContracts === 0, "Swiggy tool parity has orphan contracts");
assert(toolParityAuditor.toolParityAuditor.totals.commercialActions === 3, "Swiggy tool parity commercial action count is wrong");
assert(
  toolParityAuditor.toolParityAuditor.serverSummaries
    .map((server) => `${server.server}:${server.covered}/${server.expectedTools}`)
    .join(",") === "food:14/14,instamart:13/13,dineout:8/8",
  "Swiggy tool parity server coverage is wrong",
);
assert(
  toolParityAuditor.toolParityAuditor.rows.some(
    (row) =>
      row.id === "food_place_food_order" &&
      row.routeClass === "commercial_action" &&
      row.confirmationGate.includes("get_food_cart") &&
      row.retryPolicy.includes("Never blind-retry"),
  ),
  "Swiggy tool parity Food commercial contract is missing",
);
assert(
  toolParityAuditor.toolParityAuditor.rows.some(
    (row) => row.id === "food_report_error" && row.routeClass === "support" && row.confirmationGate.includes("Redact"),
  ),
  "Swiggy tool parity support contract is missing",
);
assert(
  toolParityAuditor.toolParityAuditor.assertions.some((assertion) => assertion.includes("Every live reference tool")) &&
    toolParityAuditor.toolParityAuditor.driftSignals.some((signal) =>
      signal.includes("Every live reference tool has a local Tool Contract Matrix row"),
    ),
  "Swiggy tool parity assertions or drift signals are missing",
);
...
assert(
  proof.proof.artifacts.some((artifact) => artifact.label === "Swiggy Builders Journey Gate Center"),
...
assert(
  launchBundle.launchBundle.artifacts.some((artifact) => artifact.label === "MCP Tool Lab") &&
    ...
    launchBundle.launchBundle.artifacts.some((artifact) => artifact.label === "Swiggy Showcase Submission Center") &&
...
assert(
  launchBundle.launchBundle.handoffEmail.body.includes("/api/swiggy-showcase-submission-center"),
  "launch bundle showcase submission handoff link is missing",
);
assert(
  ...
  JSON.stringify(
    {
      ...
      navigationWitnessScore: navigationWitness.navigationWitness.score,
      navigationWitnessRows: navigationWitness.navigationWitness.totals.rows,
      navigationWitnessManualGates: navigationWitness.navigationWitness.totals.manualGates,
      ...
      toolParityScore: toolParityAuditor.toolParityAuditor.score,
      toolParityMatched: toolParityAuditor.toolParityAuditor.totals.matchedTools,
