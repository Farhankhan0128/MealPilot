...
- Swiggy Builders Benefits Witness at `/api/swiggy-builders-benefits-witness` that proves every official What You Get promise against live APIs, quota, support, co-branding, showcase, hiring visibility, growth, enterprise support, proof links, owners, route optimizations, and Swiggy/operator gates.
- Swiggy Builders AI Native Witness at `/api/swiggy-builders-ai-native-witness` that proves the AI-native promise across agent benchmarks, AI client configs, coding-agent governance, MCP resources/prompts, multi-turn state, widgets, innovation lanes, commercial-safety boundaries, and Swiggy gates.
- Swiggy Builders Enterprise Witness at `/api/swiggy-builders-enterprise-witness` that proves the platform-operator promise across delegated OAuth, tenant controls, quota/capacity review, support SLA, contract/compliance gates, audit exports, brand governance, growth asks, and Swiggy approvals.
- Swiggy Builders Consumer Witness at `/api/swiggy-builders-consumer-witness` that proves the consumer AI-client path across AI client setup, visual dish capture, voice commerce, nutrition/budget planning, household preferences, guest collaboration, luxury workspaces, confirmation safety, proof links, and Swiggy approvals.
- Swiggy Builders Tool Reference Witness at `/api/swiggy-builders-tool-reference-witness` that proves Food 14, Instamart 13, Dineout 8, contract rows, parameters, Tool Lab probes, recipe scenarios, docs twins, and commercial-safety classes against the official Swiggy MCP references.
- Swiggy Builders Credential Sandbox Witness at `/api/swiggy-builders-credential-sandbox-witness` that proves OAuth PKCE, DCR, credential vault redaction, handoff packets, sandbox workbench, staging credential drills, seeded smoke, certification, cutover, and live-signal gates in one reviewer receipt.
- Swiggy Builders Access Policy Witness at `/api/swiggy-builders-access-policy-witness` that proves production-access fields, review checks, allowed/restricted/prohibited rules, legal gates, official CTAs, attachments, brand/data controls, and Swiggy approval ownership.
...
- Swiggy Builders Module Witness at `/api/swiggy-builders-module-witness` that witnesses every Website Atlas module with live or fallback source state, owner, CTA mapping, proof links, route optimization, risk boundary, next action, and operator or Swiggy gates.
- Swiggy Builders Navigation Witness at `/api/swiggy-builders-navigation-witness` that witnesses the global header, docs subnav, footer program links, footer resources, and footer legal links with normalized URLs, CTA live matches, proof links, owners, and manual legal/email gates.
...
- Swiggy Homepage Signal Coverage Board at `/api/swiggy-homepage-signal-coverage` that maps homepage promises, header/docs/footer links, CTAs, llms sources, support, rate-limit, growth, demo, access, FAQ, and MCP signals to MealPilot surfaces, proof links, tests, owners, and external gates.
- Swiggy Builders Completion Ledger at `/api/swiggy-builders-completion-ledger` that audits the full MealPilot objective across public site coverage, developer and enterprise tracks, all MCP servers/tools, route optimization, premium frontend, backend, logging, tracing, tests, docs, access submission, sandbox credentials, safety, reviewer packet, owners, proof routes, and remaining operator or Swiggy gates.
...
- Swiggy Source Availability Audit at `/api/swiggy-source-availability-audit` that probes public Builders pages, docs roots, and llms manifests, classifies live content versus temporary-glitch fallback, and links every source row to MealPilot proof.
- Swiggy Builders Coverage Receipt at `/api/swiggy-builders-coverage-receipt` that composes Website Atlas, Page Mesh, CTA Live Audit, Docs Coverage, Source Availability, Tool Parity, Visual QA, proof links, fallback rows, missing rows, and external gates into one reviewer receipt.
...
- Reviewer Artifact Vault that packages proof links, OpenAPI, MCP client/coding-agent readiness, source-freeze/live-site QA, executable CTA rehearsals, runtime governance/session proof, smoke commands, screenshot targets, Credential Readiness Dossier, Showcase Submission Center, redacted issuance state, demo-video checklist, logs, traces, redaction rules, support context, and Swiggy handoff copy into one access-submission manifest.
- Visual QA Center that maps reviewer screenshot targets, viewport sizes, selector manifests, text-fit and no-overlap rules, Swiggy widget fallback checks, mobile layout checks, Capability Traceability card proof, and screenshot automation gates.
...
GET  /api/swiggy-builders-navigation-witness
...
GET  /api/swiggy-builders-benefits-witness
GET  /api/swiggy-builders-ai-native-witness
GET  /api/swiggy-builders-consumer-witness
GET  /api/swiggy-builders-tool-reference-witness
GET  /api/swiggy-builders-credential-sandbox-witness
GET  /api/swiggy-builders-access-policy-witness
...
`GET /api/swiggy-source-freeze-diff` and `POST /api/swiggy-source-freeze-diff/freeze` are the final source-freeze gate before demo recording or access submission: they compose live Builders Page Mesh, Website Atlas header/footer/CTA counts, llms/docs coverage, 35-tool reference parity, Access Evidence Matrix, Access Policy Witness, Credential Sandbox Witness, Builder Packet Export, Upstream Watch, proof commands, missing inputs, and the mandatory browser re-browse gate without submitting external state. The POST flow can include a metadata-only browser re-browse receipt so the final operator visual check becomes auditable and the freeze can advance from `refresh_required` to `ready_to_freeze` when all source rows are matched.
...
`GET /api/swiggy-builders-module-witness` is the module evidence receipt: every Website Atlas module is joined to Page Mesh source state, Deep Site Map proof, owner, CTA mapping, route optimization, risk boundary, next action, and explicit live/fallback/operator/Swiggy gate status.

`GET /api/swiggy-builders-navigation-witness` is the navigation evidence receipt: the global header, docs subnav, footer program links, footer resources, and footer legal links are normalized, grouped, matched against CTA Live Audit when applicable, and marked with proof links plus manual legal/email gates.
...
`GET /api/swiggy-homepage-signal-coverage` is the public homepage promise board: header, docs subnav, footer resources/legal, Start Building, production access, demo submission, rate-limit, support, co-branding/growth, FAQ, llms, and MCP capability signals are mapped to MealPilot surfaces, proof routes, frontend visibility, docs proof, test proof, owners, and explicit operator or Swiggy gates.

`GET /api/swiggy-builders-completion-ledger` is the full-objective audit board: public website coverage, developer and enterprise tracks, all MCP servers/tools, route optimization, premium frontend, backend, logging, tracing, tests, docs, access submission, sandbox credentials, safety, reviewer packet, launch readiness, proof links, owners, and remaining operator or Swiggy gates are mapped requirement-by-requirement.
...
`GET /api/swiggy-builders-benefits-witness` is the read-only proof receipt for those same Builders "What you get" promises. It maps live API access, quota expansion, room to experiment, technical support, co-branding, showcase visibility, hiring visibility, growth partnership, and enterprise support to source URLs, owners, proof links, route optimizations, risk boundaries, grouped gates, and Launch Center evidence.

`GET /api/swiggy-builders-ai-native-witness` is the read-only proof receipt for the Builders AI-native promise. It maps agent benchmark journeys, AI client targets, coding-agent rules, MCP resources and prompts, multi-turn state orchestration, widget and hosted-widget surfaces, innovation lanes, and commercial-safety boundaries to source URLs, owners, proof links, route optimizations, risk boundaries, grouped readiness, and explicit Swiggy gates.

`GET /api/swiggy-builders-enterprise-witness` is the read-only proof receipt for the Builders enterprise/platform-operator promise. It maps delegated OAuth, tenant and end-user controls, quota and capacity review, support/SLA readiness, contract and compliance gates, audit exports, brand governance, and growth asks to source URLs, proof links, route optimizations, grouped readiness, and Swiggy-owned approvals.

`GET /api/swiggy-builders-consumer-witness` is the read-only proof receipt for the Builders consumer AI-client promise. It maps AI client setup, visual dish capture, voice commerce, nutrition and budget planning, household personalization, guest collaboration, luxury workspaces, and confirmation safety to source URLs, proof links, route optimizations, grouped readiness, and Swiggy-owned approvals.

`GET /api/swiggy-builders-tool-reference-witness` is the read-only proof receipt for the official Swiggy MCP tool reference. It maps Food 14, Instamart 13, Dineout 8, contract rows, parameters, Tool Lab probes, recipe scenarios, docs coverage, docs twins, and commercial-safety classes to source URLs, proof links, route optimizations, grouped readiness, and Swiggy-owned live credential gates.

`GET /api/swiggy-builders-credential-sandbox-witness` is the read-only proof receipt for the credential and staging path. It maps OAuth PKCE, Dynamic Client Registration, credential vault redaction, handoff readiness, sandbox credentials, first-call staging drills, seeded smoke, 35-tool certification, production cutover, live-signal calibration, and delegated auth to source URLs, proof links, route optimizations, grouped readiness, and Swiggy-owned staging credential gates.

`GET /api/swiggy-builders-access-policy-witness` is the read-only proof receipt for the production-access policy path. It maps application fields, review checks, allowed/restricted/prohibited rules, legal terms, official CTAs, attachments, browser runbook, brand and data governance, approval decision gates, proof links, route optimizations, and Swiggy-owned production access approvals.
...
`GET /api/reviewer-artifact-vault` and `POST /api/reviewer-artifact-vault/compose` are the Swiggy access-submission manifest and packet composer: proof links, OpenAPI, MCP client/coding-agent readiness, source-freeze/live-site QA, executable CTA rehearsals, runtime governance/session proof, smoke commands, screenshot targets, Access Submission Studio, Submission Timeline, Credential Readiness Dossier, redacted credential issuance state, Demo Evidence Director, Showcase Submission Center, partner/signal operations, demo-video checklist, logs, traces, redaction rules, support context, and handoff email copy are bundled in one route. The Launch Center composer accepts artifact section, channel, audience, screenshots, demo-video, and credential-gate inputs, then returns a reviewer-safe packet with included artifacts, commands, redaction rules, email copy, missing attachment gates, telemetry, and explicit no-secret assertions.

`GET /api/visual-qa-center` and `POST /api/visual-qa-center/rehearse` are the screenshot and layout evidence center: desktop, tablet, and mobile selectors, Playwright artifact paths, no-overlap rules, text-fit rules, widget fallback checks, redaction visibility, commercial confirmation visibility, Source Intelligence, Deep Site Map, Innovation Radar, Module Witness, Navigation Witness, Capability Traceability, Homepage Signal Coverage, Completion Ledger, Coverage Receipt, Source Availability, and screenshot automation gates are made reviewable. The Launch Center rehearsal accepts target group, viewport, capture mode, Swiggy-widget inclusion, and manual attachment readiness, then returns screenshot targets, rules, commands, artifact paths, readiness decision, missing capture gates, telemetry, and no-blank-render assertions.

`GET /api/swiggy-builders-coverage-receipt` is the compact all-up coverage receipt: public Builders pages, modules, CTA targets, global header, docs nav, footer resources/legal, `llms` manifests, 69 docs pages, 35 reference tools, Visual QA targets, fallback rows, missing rows, proof links, commands, assertions, and Swiggy/operator gates are reduced to one `complete_coverage`, `coverage_watch`, or `blocked_drift` decision.
...
`GET /api/builder-packet-export` and `GET /api/builder-packet-export.md` generate the executable Swiggy access packet: prepared form fields, required attachments, verification commands, local artifact paths, visual QA proof, Access Submission readiness, post-submit lifecycle readiness, Showcase Submission readiness, partner/signal readiness, handoff email copy, and explicit operator/Swiggy gates for form submission, demo video, credentials, redirect URI, and co-branding approval.
