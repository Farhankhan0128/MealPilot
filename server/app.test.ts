...
describe("MealPilot API", () => {
  ...
  it("serves security headers and OpenAPI contract", async () => {
    ...
    expect(openApi.body.paths["/api/swiggy-builders-module-witness"].get.summary).toContain("Module Witness");
    expect(openApi.body.paths["/api/swiggy-builders-module-witness"].get.responses["200"].description).toContain("source state");
    expect(openApi.body.paths["/api/swiggy-builders-navigation-witness"].get.summary).toContain("Navigation Witness");
    expect(openApi.body.paths["/api/swiggy-builders-navigation-witness"].get.responses["200"].description).toContain("footer legal");
    expect(openApi.body.paths["/api/swiggy-builders-benefits-witness"].get.summary).toContain("Benefits Witness");
    expect(openApi.body.paths["/api/swiggy-builders-benefits-witness"].get.responses["200"].description).toContain("co-branding");
    expect(openApi.body.paths["/api/swiggy-builders-ai-native-witness"].get.summary).toContain("AI Native Witness");
    ...
    expect(openApi.body.paths["/api/swiggy-partner-success-desk/compose"].post.summary).toContain(
  ...
  it("composes a Swiggy Partner Success handoff packet for a ready capacity lane", async () => {
    const { app } = createMealPilotServer();
    await request(app).post("/api/plan").send(planningRequest).expect(201);
    ...
    const response = await request(app)
      ...
      .expect(200);
    const packet = response.body.partnerSuccessHandoff;

    expect(packet.decision).toBe("ready_local_handoff");
    expect(packet.readinessScore).toBe(100);
    expect(packet.lane.id).toBe("traffic_capacity");
    expect(packet.escalationEmail.id).toBe("capacity");
    expect(packet.missingInputs).toEqual([]);
    expect(packet.proofLinks).toEqual(expect.arrayContaining(["/api/swiggy-partner-success-desk", "/api/traffic-readiness-plan"]));
    expect(packet.handoffDraft.to).toBe("builders@swiggy.in");
    expect(packet.handoffDraft.bodyPreview).toContain("Capacity handoff for Swiggy Builders review");
    expect(packet.checklist.every((item: { status: string }) => item.status === "ready")).toBe(true);
    expect(packet.assertions.some((assertion: string) => assertion.includes("never sends email"))).toBe(true);
  });

  it("keeps Partner Success handoff in operator input state when required context is missing", async () => {
    const { app } = createMealPilotServer();

    const response = await request(app)
      .post("/api/swiggy-partner-success-desk/compose")
      .send({ laneId: "developer_support", operatorEmail: "bad-email", launchWindow: "", contextNote: "" })
      .expect(200);
    const packet = response.body.partnerSuccessHandoff;

    expect(packet.decision).toBe("needs_operator_input");
    expect(packet.readinessScore).toBe(68);
    expect(packet.lane.id).toBe("developer_support");
    expect(packet.missingInputs).toEqual(expect.arrayContaining(["operator_email", "launch_window", "context_note"]));
    expect(packet.checklist.some((item: { status: string }) => item.status === "manual_input")).toBe(true);
  });

  it("preserves Swiggy gates for enterprise Partner Success handoffs", async () => {
    const { app } = createMealPilotServer();
    ...
    const response = await request(app)
      ...
      .send({
        ...
        contextNote: "Requesting partner manager readiness review.",
      })
      .expect(200);
    const packet = response.body.partnerSuccessHandoff;
    ...
    expect(packet.lane.id).toBe("enterprise_slack_partner");
    expect(packet.checklist.some((item: { id: string; status: string }) => item.id === "swiggy_partner_gate_preserved" && item.status === "external_gate")).toBe(true);
    expect(packet.externalGates.some((gate: string) => gate.includes("Slack"))).toBe(true);
  });
  ...
  it("returns a reviewer artifact vault for Swiggy access submission", async () => {
    ...
    expect(
      vault.artifactSections.some((section: { artifacts: Array<{ id: string; label: string; path: string }> }) =>
        section.artifacts.some(
          (artifact) =>
            artifact.id === "access_submission_studio" &&
            artifact.label === "Access Submission Studio" &&
    ...
    expect(
      vault.artifactSections.some((section: { artifacts: Array<{ id: string; label: string; path: string }> }) =>
        section.artifacts.some(
          (artifact) =>
            artifact.id === "showcase_submission_center" &&
            artifact.label === "Swiggy Showcase Submission Center" &&
            artifact.path === "/api/swiggy-showcase-submission-center",
        ),
      ),
    ).toBe(true);
    expect(
    ...
    expect(vault.reviewerEmail.body).toContain("/api/swiggy-showcase-submission-center");
    ...
    expect(vault.reviewerEmail.body).toContain("/api/swiggy-partner-success-desk");
    expect(vault.reviewerEmail.body).toContain("/api/traffic-readiness-plan");
    expect(vault.reviewerEmail.body).toContain("/api/swiggy-quota-negotiation-center");
    expect(vault.reviewerEmail.body).toContain("/api/data-governance-center");
    expect(vault.reviewerEmail.body).toContain("/api/swiggy-live-signal-calibration");
  ...
  it("returns visual QA evidence for reviewer screenshots and responsive layout", async () => {
    ...
    expect(
      visualQa.targetGroups.some((group: { targets: Array<{ id: string; selector: string; viewport: string }> }) =>
        group.targets.some(
          (target) =>
            target.id === "module_witness_card" &&
            target.selector === ".module-witness-card" &&
            target.viewport === "desktop",
        ),
      ),
    ).toBe(true);
    expect(
      visualQa.targetGroups.some((group: { targets: Array<{ id: string; selector: string; viewport: string }> }) =>
        group.targets.some(
          (target) =>
            target.id === "navigation_witness_card" &&
            target.selector === ".navigation-witness-card" &&
            target.viewport === "desktop",
        ),
      ),
    ).toBe(true);
    expect(
      visualQa.targetGroups.some((group: { targets: Array<{ id: string; selector: string; viewport: string }> }) =>
        group.targets.some(
          (target) =>
            target.id === "benefits_witness_card" &&
            target.selector === ".benefits-witness-card" &&
            target.viewport === "desktop",
        ),
      ),
    ).toBe(true);
    expect(
      visualQa.targetGroups.some((group: { targets: Array<{ id: string; selector: string; viewport: string }> }) =>
        group.targets.some(
          (target) =>
            target.id === "ai_native_witness_card" &&
            target.selector === ".ai-native-witness-card" &&
            target.viewport === "desktop",
        ),
      ),
    ).toBe(true);
  ...
  it("returns Swiggy Builders Module Witness for every Website Atlas module", async () => {
    const { app, config } = createMealPilotServer();
    const witness = await buildSwiggyBuildersModuleWitness({
      config,
      fetchPage: async () => ({
        ok: true,
        statusCode: 200,
        durationMs: 8,
        text: "<html><title>Swiggy</title><body>We are fixing a temporary glitch.</body></html>",
      }),
    });

    expect(witness.score).toBeGreaterThanOrEqual(80);
    expect(witness.decision).toBe("module_witness_watch");
    expect(witness.totals.modules).toBeGreaterThanOrEqual(38);
    expect(witness.totals.pages).toBeGreaterThanOrEqual(8);
    expect(witness.totals.fallback).toBeGreaterThanOrEqual(1);
    expect(witness.totals.blocked).toBe(0);
    expect(
      witness.totals.proven +
        witness.totals.fallback +
        witness.totals.watch +
        witness.totals.operatorGates +
        witness.totals.swiggyGates +
        witness.totals.blocked,
    ).toBe(witness.totals.modules);
    expect(witness.totals.ctaMappedModules).toBeGreaterThanOrEqual(30);
    expect(witness.totals.proofLinks).toBeGreaterThanOrEqual(20);
    expect(witness.rows.map((row: { id: string }) => row.id)).toEqual(
      expect.arrayContaining(["home_hero", "developers_toolkit", "access_legal_framework", "reference_food"]),
    );
    expect(
    ...
    expect(witness.pageSummaries.some((summary: { pageId: string; modules: number }) => summary.pageId === "home" && summary.modules >= 6)).toBe(true);
    expect(witness.commands.some((command: { command: string }) => command.command.includes("/api/swiggy-builders-module-witness"))).toBe(true);
    expect(witness.assertions.some((assertion: string) => assertion.includes("Every Website Atlas module"))).toBe(true);
    expect(witness.externalGates.some((gate: string) => gate.includes("External forms"))).toBe(true);

    const response = await request(app).get("/api/swiggy-builders-module-witness").expect(200);
    expect(response.body.moduleWitness.totals.modules).toBeGreaterThanOrEqual(38);
    expect(response.body.moduleWitness.totals.blocked).toBe(0);
  });

  it("returns Swiggy Builders Navigation Witness for header, docs, and footer links", async () => {
    const { app, config } = createMealPilotServer();
    const witness = await buildSwiggyBuildersNavigationWitness({
      config,
      probeTarget: async () => ({ ok: true, statusCode: 200, durationMs: 6 }),
    });

    expect(witness.score).toBeGreaterThanOrEqual(90);
    expect(witness.decision).toBe("navigation_ready");
    expect(witness.totals.rows).toBeGreaterThanOrEqual(20);
    expect(witness.totals.headerLinks).toBe(7);
    expect(witness.totals.docsLinks).toBe(5);
    expect(witness.totals.footerLinks).toBeGreaterThanOrEqual(8);
    expect(witness.totals.blocked).toBe(0);
    expect(witness.rows.map((row: { id: string }) => row.id)).toEqual(
      expect.arrayContaining(["global_header_home", "docs_subnav_docs_reference", "footer_legal_privacy", "footer_resources_llms"]),
    );
    expect(witness.rows.some((row: { id: string; status: string }) => row.id === "footer_legal_privacy" && row.status === "manual_gate")).toBe(true);
    expect(witness.groups.map((group: { id: string }) => group.id)).toEqual(
      expect.arrayContaining(["global_header", "docs_subnav", "footer_program", "footer_resources", "footer_legal"]),
    );
    expect(witness.commands.some((command: { command: string }) => command.command.includes("/api/swiggy-builders-navigation-witness"))).toBe(true);
    expect(witness.assertions.some((assertion: string) => assertion.includes("global header"))).toBe(true);
    expect(witness.externalGates.some((gate: string) => gate.includes("Legal review"))).toBe(true);

    const response = await request(app).get("/api/swiggy-builders-navigation-witness").expect(200);
    expect(response.body.navigationWitness.totals.rows).toBeGreaterThanOrEqual(20);
    expect(response.body.navigationWitness.totals.blocked).toBe(0);
  });

  it("returns Swiggy Builders Benefits Witness for official What You Get promises", async () => {
    const { app } = createMealPilotServer();
    const response = await request(app).get("/api/swiggy-builders-benefits-witness").expect(200);
    const witness = response.body.benefitsWitness;

    expect(witness.score).toBeGreaterThanOrEqual(80);
    expect(witness.decision).toBe("benefits_watch");
    expect(witness.totals.rows).toBeGreaterThanOrEqual(9);
    expect(witness.totals.proven).toBeGreaterThanOrEqual(2);
    expect(witness.totals.swiggyGates).toBeGreaterThanOrEqual(2);
    expect(witness.totals.proofLinks).toBeGreaterThanOrEqual(20);
    expect(witness.totals.swiggyMcpServers).toBe(3);
    expect(witness.totals.officialTools).toBe(35);
    expect(witness.totals.activationCtas).toBe(6);
    expect(witness.rows.map((row: { id: string }) => row.id)).toEqual(
      expect.arrayContaining([
        "live_api_access",
        "quota_expansion",
    ...
    expect(witness.groups.map((group: { id: string }) => group.id)).toEqual(
      expect.arrayContaining(["platform", "operations", "growth"]),
    );
    expect(witness.commands.some((command: { command: string }) => command.command.includes("/api/swiggy-builders-benefits-witness"))).toBe(true);
    expect(witness.assertions.some((assertion: string) => assertion.includes("What You Get"))).toBe(true);
    expect(witness.externalGates.some((gate: string) => gate.includes("production credentials"))).toBe(true);
  });

  it("returns Swiggy Builders AI Native Witness for agents, clients, prompts, widgets, and safety", async () => {
    const { app } = createMealPilotServer();
    const response = await request(app).get("/api/swiggy-builders-ai-native-witness").expect(200);
    const witness = response.body.aiNativeWitness;

    expect(witness.score).toBeGreaterThanOrEqual(88);
    expect(witness.decision).toBe("ai_native_watch");
    expect(witness.totals.rows).toBeGreaterThanOrEqual(8);
    expect(witness.totals.proven).toBeGreaterThanOrEqual(4);
    expect(witness.totals.officialTools).toBe(35);
    expect(witness.totals.benchmarkJourneys).toBeGreaterThanOrEqual(8);
    expect(witness.totals.clientTargets).toBeGreaterThanOrEqual(6);
    expect(witness.totals.resources).toBe(6);
    expect(witness.totals.prompts).toBe(6);
    expect(witness.totals.widgetSurfaces).toBeGreaterThanOrEqual(7);
    expect(witness.totals.proofLinks).toBeGreaterThanOrEqual(20);
    expect(witness.rows.map((row: { id: string }) => row.id)).toEqual(
      expect.arrayContaining([
    ...
    expect(witness.groups.map((group: { id: string }) => group.id)).toEqual(
      expect.arrayContaining(["agent_runtime", "client_connect", "experience_surfaces", "innovation_governance"]),
    );
    expect(witness.commands.some((command: { command: string }) => command.command.includes("/api/swiggy-builders-ai-native-witness"))).toBe(true);
    expect(witness.assertions.some((assertion: string) => assertion.includes("AI-native"))).toBe(true);
    expect(witness.externalGates.some((gate: string) => gate.includes("hosted widget"))).toBe(true);
  });

  it("returns Swiggy Builders Enterprise Witness for platform, delegated auth, quota, support, and governance", async () => {
    const { app } = createMealPilotServer();
    const response = await request(app).get("/api/swiggy-builders-enterprise-witness").expect(200);
    const witness = response.body.enterpriseWitness;

    expect(witness.score).toBeGreaterThanOrEqual(82);
    expect(["enterprise_ready", "enterprise_watch", "enterprise_blocked"]).toContain(witness.decision);
    expect(witness.currentTrack).toBe("developer_ready_enterprise_planned");
    expect(witness.totals.rows).toBeGreaterThanOrEqual(8);
    expect(witness.totals.delegatedFlowSteps).toBeGreaterThanOrEqual(8);
    expect(witness.totals.onboardingSteps).toBeGreaterThanOrEqual(6);
    expect(witness.totals.platformLanes).toBeGreaterThanOrEqual(7);
    expect(witness.totals.tenantControls).toBeGreaterThanOrEqual(5);
    expect(witness.totals.supportLanes).toBeGreaterThanOrEqual(5);
    expect(witness.totals.contractGates).toBeGreaterThanOrEqual(4);
    expect(witness.totals.auditExports).toBeGreaterThanOrEqual(3);
    expect(witness.totals.operatingPillars).toBeGreaterThanOrEqual(6);
    expect(witness.totals.operatingRunbooks).toBeGreaterThanOrEqual(4);
    expect(witness.totals.partnershipAsks).toBeGreaterThanOrEqual(6);
    expect(witness.totals.proofLinks).toBeGreaterThanOrEqual(20);
    expect(witness.rows.map((row: { id: string }) => row.id)).toEqual(
      expect.arrayContaining([
    ...
    expect(witness.groups.map((group: { id: string }) => group.id)).toEqual(
      expect.arrayContaining(["platform_auth", "operations_support", "governance_commercial", "audit_launch"]),
    );
    expect(witness.commands.some((command: { command: string }) => command.command.includes("/api/swiggy-builders-enterprise-witness"))).toBe(true);
    expect(witness.assertions.some((assertion: string) => assertion.includes("Enterprise access"))).toBe(true);
  ...
  it("returns Swiggy Builders Tool Reference Witness for all official MCP tools and docs coverage", async () => {
    const { app } = createMealPilotServer();
    const response = await request(app).get("/api/swiggy-builders-tool-reference-witness").expect(200);
    const witness = response.body.toolReferenceWitness;

    expect(witness.score).toBeGreaterThanOrEqual(90);
    expect(["tool_reference_ready", "tool_reference_watch", "tool_reference_blocked"]).toContain(witness.decision);
    expect(witness.totals.rows).toBeGreaterThanOrEqual(8);
    expect(witness.totals.servers).toBe(3);
    expect(witness.totals.officialTools).toBe(35);
    expect(witness.totals.foodTools).toBe(14);
    expect(witness.totals.instamartTools).toBe(13);
    expect(witness.totals.dineoutTools).toBe(8);
    expect(witness.totals.contractRows).toBe(35);
    expect(witness.totals.contractParameters).toBeGreaterThanOrEqual(40);
    expect(witness.totals.toolLabProbes).toBe(35);
    expect(witness.totals.scenarioToolsCovered).toBe(35);
    expect(witness.totals.docsPages).toBeGreaterThanOrEqual(69);
    expect(witness.totals.commercialTools).toBe(3);
    expect(witness.totals.proofLinks).toBeGreaterThanOrEqual(18);
    expect(witness.rows.map((row: { id: string }) => row.id)).toEqual(
      expect.arrayContaining([
    ...
    expect(witness.groups.map((group: { id: string }) => group.id)).toEqual(
      expect.arrayContaining(["server_references", "executable_contracts", "docs_and_drift", "commercial_safety"]),
    );
    expect(witness.commands.some((command: { command: string }) => command.command.includes("/api/swiggy-builders-tool-reference-witness"))).toBe(true);
    expect(witness.assertions.some((assertion: string) => assertion.includes("Food, Instamart, and Dineout"))).toBe(true);
